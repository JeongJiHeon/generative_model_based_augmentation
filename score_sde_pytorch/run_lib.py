...
def train(config, workdir):
  ...
  train_ds, eval_ds = dataset_T1FLAIR.get_loader(config)
  ...
  for step in range(initial_step, num_train_steps + 1):
    try:
      batch = train_iter.next().to(config.device)
    except:
    ...
    if step % config.training.eval_freq == 0:
      try:
        eval_batch = eval_iter.next().to(config.device)
      except:
